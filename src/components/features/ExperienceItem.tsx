...
export const ExperienceItem = ({
  ...
  // compute earliest start and latest end across roles
  ...
  const earliestStart = data.roles.reduce((acc, r) => {
    const val = toComparable(r.startDate);
    return acc === null || val < acc.val ? { val, str: r.startDate } : acc;
  }, null as null | { val: number; str: string });
  const latestEnd = data.roles.reduce((acc, r) => {
    if (!r.endDate || r.endDate.trim() === "") return acc;
    const val = toComparable(r.endDate);
    return acc === null || val > acc.val ? { val, str: r.endDate } : acc;
  }, null as null | { val: number; str: string });

  const startLabel = earliestStart?.str ?? "";
  const endLabel = hasPresent ? "Present" : latestEnd?.str ?? "";
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] gap-4 md:gap-8">
        ...
        <div className="hidden md:flex flex-col items-center relative">
          ...
          {!isLast && (
            <div className="absolute top-5 -bottom-8 w-px bg-linear-to-b from-purple-500/50 to-transparent"></div>
          )}
        </div>
